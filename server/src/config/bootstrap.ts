...
export async function initialiseOpenAIKeys(): Promise<void> {
  ...
  if (existing) {
    logger.info(`[bootstrap] OpenAI API key with alias "${alias}" already exists; skipping seeding.`);
    return;
  }
  ...
  try {
    ...
    logger.info(`[bootstrap] Stored OpenAI API key with alias "${alias}" from environment.`);
  } catch (error) {
    ...
    logger.error({ err: error }, '[bootstrap] Failed to store OpenAI API key from environment');
  }
}
